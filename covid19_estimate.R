...
for(m_sparse in c(14,21,28,35)){
  ...
  for(i in 1:itermax){
   ...
   cat(c("m'=",m_sparse,round(mean(hatbeta_sparse_result),4),round(sd(hatbeta_sparse_result)/sqrt(itermax),4),round(mean(hatgamma_sparse_result),4),round(sd(hatgamma_sparse_result),4),round(mean(hatrho_sparse_result),4),round(sd(hatrho_sparse_result),4),"\n"))
}
cat(c("original data",round(hatbeta,4),round(hatgamma,4),round(hatrho,4)),"\n")
